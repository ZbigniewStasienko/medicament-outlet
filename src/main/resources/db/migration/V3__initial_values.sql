...
INSERT INTO Pharmacy (id, picture_id, address, phone_number, name, email, opening_hours) VALUES
    (gen_random_uuid(), (SELECT id FROM Pictures LIMIT 1), '123 Main St', '123-456-7890', 'Pharmacy One', 'contact@pharmacyone.com', '9:00 - 18:00'),
    (gen_random_uuid(), (SELECT id FROM Pictures OFFSET 1 LIMIT 1), '456 Side Ave', '987-654-3210', 'Pharmacy Two', 'support@pharmacytwo.com', '10:00 - 19:00');

INSERT INTO Medicine (id, pharmacy_id, picture_id, name, description, size) VALUES
    (gen_random_uuid(), (SELECT id FROM Pharmacy LIMIT 1), (SELECT id FROM Pictures LIMIT 1), 'Aspirin', 'Pain relief medicine', 100),
    (gen_random_uuid(), (SELECT id FROM Pharmacy OFFSET 1 LIMIT 1), (SELECT id FROM Pictures OFFSET 1 LIMIT 1), 'Ibuprofen', 'Anti-inflammatory medicine', 50);
...
INSERT INTO ReservedProducts (id, product_id, reservation_id, amount) VALUES
